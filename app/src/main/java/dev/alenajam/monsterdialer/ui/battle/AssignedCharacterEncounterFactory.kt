...
class AssignedCharacterEncounterFactory(
    ...
    fun forCall(callId: String, contactKey: String, callerName: String, isAnonymous: Boolean): BattleEncounter {
        ...
        val player = assignments.player()
            ?.let { characters.find(it, CharacterAssignmentTarget.Player) }
            ?.asBattlePokemon()
            ?: fallback.player
        val enemy = if (isAnonymous) {
            fallback.enemy
        } else {
            assignments.characterForContact(contactKey)
                ?.let { characters.find(it, CharacterAssignmentTarget.Contact) }
                ?.asBattlePokemon()
                ?: fallback.enemy
        }
        return fallback.copy(player = player, enemy = enemy)
    }
